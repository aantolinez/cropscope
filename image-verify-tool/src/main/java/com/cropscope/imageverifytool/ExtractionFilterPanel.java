...
public class ExtractionFilterPanel extends JPanel {
    ...
    private static boolean nonEmpty(JTextField tf){ return tf.getText()!=null && tf.getText().trim().length()>0; }
    private static Integer parseInt(String s){ try { return Integer.valueOf(s.trim()); } catch(Exception e){ return null; } }
    private static Double  parseDouble(String s){ try { return Double.valueOf(s.trim()); } catch(Exception e){ return null; } }
}
